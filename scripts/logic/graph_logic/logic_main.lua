...
hot_lava_location = {}
hot_lava_location.__index = hot_lava_location
...
function hot_lava_location.new(name)
    local self = setmetatable({}, hot_lava_location)
    if name then
...
function hot_lava_location:connect_one_way(exit, rule)
    if type(exit) == "string" then
        ...
        if existing then
            ...
            exit = hot_lava_location.new(exit)
        end
    end
    if rule == nil then
        rule = always
...
function hot_lava_location:connect_two_ways(exit, rule)
...
function hot_lava_location:connect_one_way_entrance(name, exit, rule)
    if rule == nil then
        rule = always
    end
    self.exits[#self.exits + 1] = { exit, rule }
...
function hot_lava_location:connect_two_ways_entrance(name, exit, rule)
...
-- acts as a shortcut for 2 connect_one_way_entrance-calls
function hot_lava_location:connect_two_ways_entrance_door_stuck(name, exit, rule1, rule2)
...
function hot_lava_location:connect_two_ways_stuck(exit, rule1, rule2)
...
function hot_lava_location:accessibility()
...
-- 
function hot_lava_location:discover(accessibility, keys)
...
entry_point = hot_lava_location.new("entry_point")
